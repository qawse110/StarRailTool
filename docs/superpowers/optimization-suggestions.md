...
> **生成日期：** 2026-07-04（最后更新：2026-07-04 v2）
...
| 优先级 | 项目 | 影响 | 工作量 |
|--------|------|------|--------|
| 🔴 高 | DES 占位符伤害公式（✅ 已修复 v2） | 战斗模拟不真实 | 中 |
| 🔴 高 | FollowUpRule 自动追击双倍伤害（✅ 已修复 v2） | 伤害翻倍 | 小 |
| 🟡 中 | `BuffEvaluator` 静默丢弃 4 类型 | 潜在功能缺失 | 小 |
| 🟡 中 | `ScoringEngine` 单人模拟算配队分（✅ 已修复 v2） | 评分不准 | 中 |
| 🟡 中 | DOT/追击未接通 DamageCalculator（✅ 已修复 v2） | 伤害不真实 | 小 |
| 🟢 低 | 缺少 Gradle Wrapper（✅ 已修复 v2） | 开发体验 | 小 |
| 🟢 低 | Room Schema 导出未配置 | 维护性 | 微小 |
| 🟢 低 | 缺少 SP 管理/AoE 终技（✅ 已修复 v2） | 模拟不完整 | 小 |
